...
def run_experiment(pupil_shape, N, inr=False):
    ...
    actual_noise_power = torch.mean(
        (noise / torch.norm(noise, dim=(-1, -2), keepdim=True) * a) ** 2, dim=(-1, -2)
    )
    actual_noise_power = torch.mean((noisy_psfs - psfs) ** 2, dim=(-1, -2))
    ...
    actual_signal_power = torch.mean(
        psfs[:, N // 2 - 30 : N // 2 + 30, N // 2 - 30 : N // 2 + 30] ** 2, dim=(-1, -2)
    )
    actual_snr_db = 10 * torch.log10(actual_signal_power / actual_noise_power)
    print(f"Average resulting SNR: {torch.mean(actual_snr_db).item()} dB")

    # normalized_noise = noise / torch.norm(noise, dim=(-1, -2), keepdim=True)
    # scaled_noise = normalized_noise * a

    # # Calculate the power of your scaled noise
    # scaled_noise_power = torch.mean(scaled_noise**2)

    # # Calculate the resulting SNR in dB
    # signal_power = torch.mean(psfs**2)
    # resulting_snr_db = 10 * torch.log10(signal_power / scaled_noise_power)
    # print(f"Resulting SNR from your method: {resulting_snr_db.item()} dB")
    ...
    # # Projection methods
    # Gerchberg-Saxton
    hio_noisy = noisy_psfs * 0.0
    hio_noisy[:, N // 2 - 30 : N // 2 + 30, N // 2 - 30 : N // 2 + 30] = (
        noisy_psfs[:, N // 2 - 30 : N // 2 + 30, N // 2 - 30 : N // 2 + 30] * 1.0
    )
    out = gerchberg_saxton(torch.sqrt(hio_noisy), pupil.unsqueeze(0), 500)
    ...
    out = fienup_hio(torch.sqrt(hio_noisy), pupil.unsqueeze(0), 500, beta=0.5)
    psf_hat = psf(pupil, out)
    print(
    ...
    hio_noisy_dataset = torch.utils.data.TensorDataset(
        hio_noisy,
        phases,
    )
    if inr:
        ...
        phases_inr = []
        k = 0
        for psf_idx, phase in hio_noisy_dataset:
